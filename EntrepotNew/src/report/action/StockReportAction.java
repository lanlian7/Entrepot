...
@Scope("prototype")  
public class StockReportAction extends SuperAction {  
    ...
    public String exportExcel() throws RowsExceededException, WriteException, IOException{
        ...
        ///创建可写入的Excel工作表  
            ...
            WritableSheet sheet=wk.createSheet("库存信息表", 0);  
        ...
        Label lab_31=new Label(3,1,"数量",cloumnTitleFormat);  
        ...
        List<Stock> list=dao.queryAllStock();
        
        for(int i=0;i<list.size();i++){
      	  sheet.addCell(new Label(0,i+2,list.get(i).getMaterial_Number(),cloumnTitleFormat));
      	  sheet.addCell(new Label(1,i+2,list.get(i).getName().toString(),datewcf));
          sheet.addCell(new Label(2,i+2,list.get(i).getSpecification(),cloumnTitleFormat));
          ...
          sheet.addCell(new Label(7,i+2,Double.toString(list.get(i).getUnitPrice()),new WritableCellFormat(new NumberFormat("#.##") )));
        }
