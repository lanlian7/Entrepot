...
@Scope("prototype")  
public class OutGoingReportAction extends SuperAction {  
    ...
    public String exportExcel() throws RowsExceededException, WriteException, IOException{
        ...
        //获得输出流，该输出流的输出介质是客户端浏览器  
          ...
          response.setHeader("Content-disposition","attachment;           filename=OutGoing.xls");  
        ...
        ///创建可写入的Excel工作表  
            ...
            WritableSheet sheet=wk.createSheet("出库信息表", 0);  
        ...
        Label lab_00=new Label(0,0,"出库信息一览表",titleFormat);  
        ...
        Label lab_01=new Label(0,1,"出库单号",cloumnTitleFormat);   
        Label lab_11=new Label(1,1,"出库日期",cloumnTitleFormat);  
        Label lab_21=new Label(2,1,"料号",cloumnTitleFormat);    
        Label lab_31=new Label(3,1,"名称",cloumnTitleFormat);  
        Label lab_41=new Label(4,1,"规格",cloumnTitleFormat);  
        Label lab_51=new Label(5,1,"数量",cloumnTitleFormat);  
        Label lab_61=new Label(6,1,"单位",cloumnTitleFormat);  
        Label lab_71=new Label(7,1,"用途",cloumnTitleFormat);  
        Label lab_81=new Label(8,1,"领料部门",cloumnTitleFormat);  
        Label lab_91=new Label(9,1,"领料人",cloumnTitleFormat);  
        ...
        List<OutGoing> list=dao.queryAllOutGoing();
        
        for(int i=0;i<list.size();i++){
      	  sheet.addCell(new Label(0,i+2,list.get(i).getOutGoingID(),cloumnTitleFormat));
      	  sheet.addCell(new Label(1,i+2,list.get(i).getOutDate().toString(),datewcf));
          sheet.addCell(new Label(2,i+2,list.get(i).getMaterial_Number(),cloumnTitleFormat));
          sheet.addCell(new Label(3,i+2,list.get(i).getName(),cloumnTitleFormat));
          sheet.addCell(new Label(4,i+2,list.get(i).getSpecification(),cloumnTitleFormat));
          sheet.addCell(new Label(5,i+2,Long.toString(list.get(i).getNumber()),new WritableCellFormat(new NumberFormat("#") )));
          sheet.addCell(new Label(6,i+2,list.get(i).getUnit(),cloumnTitleFormat));
          sheet.addCell(new Label(7,i+2,list.get(i).getPurpose(),cloumnTitleFormat));
          sheet.addCell(new Label(8,i+2,list.get(i).getMaterial_Department(),cloumnTitleFormat));
          sheet.addCell(new Label(9,i+2,list.get(i).getMaterial_Person(),cloumnTitleFormat));
        }
